...
export default function RepositoryDetail({ match }) {
  ...
  const fetchData = useCallback(async () => {
    const response = await fetch(
      API_URL + "/repos/" + match.params.owner + "/" + match.params.repo
    );
    const data = await response.json();
    setRepository(data);
  }, [match.params.owner, match.params.repo]);

  const fetchRepositoriesFromAuthor = useCallback(async () => {
    const response = await fetch(
      API_URL + "/users/" + match.params.owner + "/repos"
    );
    const data = await response.json();

    console.log(data);
    setRepositoriesFromAuthor(data);
  }, [match.params.owner]);

  useEffect(() => {
    fetchData();
    fetchRepositoriesFromAuthor();
  }, [fetchData, fetchRepositoriesFromAuthor]);
                ...
                <div className="card-body">
                  ...
                  <div className="info">
                    ...
                    <div className="info-row">
                      ...
                      <div className="info-col">Created at 4 hours ago</div>
                    </div>
                  </div>
